    ...
    <div className="p-4 max-w-3xl mx-auto">
      ...
      <div className="bg-transparent p-4 rounded-lg mb-4 border border-gray-200/20">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {loading ? (
              <span>
                Summarizing<span className="inline-block ml-2 cursor">&nbsp;</span>
              </span>
            ) : (
              "Summarize PDF"
            )}
          </button>
          <div className="text-sm text-gray-500">Tip: paste a PDF URL or upload your file</div>
        </div>
      </div>
      ...
      {summary && (
        <div className="mt-4 p-4 border bg-transparent whitespace-pre-wrap rounded-lg">
          <strong className="block mb-2">Summary:</strong>
          <pre className="text-sm overflow-auto max-h-72">{summary}</pre>
        </div>
      )}
    </div>
  );
